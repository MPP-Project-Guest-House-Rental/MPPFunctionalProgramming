...
/**Dave 8 - Country that have The Highest average price per day in the world*/
    ...
    static Function<Country, Optional<Double>> highestAveragePriceInTheWorld= (country) ->
            Stream.of(country)
                    ...
                    .map(role -> (Guest) role)
                    .flatMap(guest -> guest.getReservations().stream())
                    .collect(Collectors.groupingBy(reservation -> reservation.getGuest().getUser().getCity().getCountry()
                            ,Collectors.summingDouble(res->averagePerNightPriceInACountry.apply(country).getAverage())))
                    .entrySet()
                    .stream()
